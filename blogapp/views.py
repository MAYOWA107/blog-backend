...
def delete_blog(request, pk):
    ...
    blog = Blog.objects.get(id=pk)
    if blog.author != user:
        return Response ({'error': 'You are not the owner of this blog'}, status=status.HTTP_403_FORBIDDEN)
    blog.delete()
    return Response({'message': 'Your post has been deleted successfully'}, status=status.HTTP_204_NO_CONTENT)
